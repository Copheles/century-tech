...
const partners = ['AMP', 'Systimax', 'Nexans', 'Panduit', 'Datwyler', 'R&M']

const standardHighlights = [
  {
    icon: BadgeCheck,
    title: 'Quality management',
    items: ['ISO 9001:2015 accreditation'],
  },
  {
    icon: Building2,
    title: 'BCA registrations',
    items: [
      'ME04 Communications and Security System - L4',
      'ME10 Line Plant Cabling/Wiring for Telecommunications - L1',
    ],
  },
  {
    icon: FileCheck2,
    title: 'Professional credentials',
    items: ['RCDD from BICSI', 'CDCP from EPI', 'BICSI Corporate Member'],
  },
]
...
function About() {
  ...
  return (
    <>
      ...
      <RevealSection className="content-section section-tint">
        <Container className="about-standards-ledger">
          <div className="about-standards-ledger-head">
            <SectionTitle
              eyebrow="Standards"
              title="Awards, certifications, and professional memberships."
              description="Our certifications reflect a long-term commitment to quality standards, compliance, and professional cabling practice."
            />
            <motion.div
              className="about-standards-chips"
              variants={staggerContainer}
              initial="hidden"
              whileInView="show"
              viewport={{ once: true, amount: 0.25 }}
            >
              {standardHighlights.map(({ icon: Icon, title }) => (
                <motion.span key={title} variants={cardIn}>
                  <Icon size={20} />
                  {title}
                </motion.span>
              ))}
          ...
          <motion.div
            className="about-standards-ledger-list"
            variants={staggerContainer}
            initial="hidden"
            whileInView="show"
            viewport={{ once: true, amount: 0.18 }}
          >
            {standardHighlights.map(({ icon: Icon, title, items }) => (
              <motion.article
                className="about-standards-ledger-row"
                key={title}
                variants={cardIn}
                whileHover={reduceMotion ? undefined : { x: 8 }}
                transition={{ type: 'spring', stiffness: 240, damping: 24 }}
              >
                <div className="about-standards-row-title">
                  <Icon size={21} />
                  <h3>{title}</h3>
                </div>
                <div className="about-standards-row-content">
                  {items.map((item) => (
            ...
            >
              ...
              <div className="about-standards-row-content">
                <span>EPU registrations for AVP, CMP, and CNE categories</span>
                <span>IDA Licensed Wiring Contractor</span>
              </div>
            </motion.article>
            ...
            >
              ...
              <div className="about-partner-list">
